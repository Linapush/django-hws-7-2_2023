...
from .forms import AddFundsForm, RegistrationForm, AddTracksForm
...
def add_tracks(request):
    if request.method == 'POST' and 'tracks' in request.POST and 'client_id' in request.POST:
        selected_tracks = request.POST.getlist('tracks')
        client_id = request.POST['client_id']
        client = Client.objects.get(id=client_id)
        for track_id in selected_tracks:
            track = Tracks.objects.get(id=track_id)
            client.tracks.add(track)
        client.save()
        return HttpResponseRedirect(reverse('client_detail', args=[client_id]))
    clients = Client.objects.all()
    tracks = Tracks.objects.filter(client=None)
    return render(request, 'add_tracks.html', {'clients': clients, 'tracks': tracks})